...
class NN(object):
    ...
    def train(self, train_counts=None, log_flag=False):
        ...
        for i in range(0, train_counts):
            ...
            # #如果cost 不在减小，退出
            # if not self.cost_last:
            #     self.cost_last = cost
            # else:
            #     self.cost_last =  self.cost_this
            #     self.cost_this = cost
            #
            # if self.cost_last and self.cost_this and (self.cost_this > self.cost_last):
            #     print 'get the minst cost return', self.cost_this, self.cost_last
            #     return
